<?php
/**
 * The template for displaying the footer
 *
 * Contains the closing of the #content div and all content after.
 *
 * @link https://developer.wordpress.org/themes/basics/template-files/#template-partials
 *
 * @package shalom-tek
 */

?>
<!-- footer section -->
<div class="container-fluid py-5 section-5 font-poppins bg-footer-black">




<div class="row px-lg-4 pb-lg-4  ">
   <div class="col-lg-6 social-media d-none d-sm-block">
	  <ul>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
				  <path
					 d="M24 12.073C24 5.405 18.628 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047v-2.66c0-3.025 1.79-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.971H15.83c-1.49 0-1.955.931-1.955 1.886v2.264h3.328l-.532 3.49h-2.796V24C19.613 23.095 24 18.1 24 12.073z">
				  </path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
				  <path
					 d="M12 0C5.37 0 0 5.37 0 12s5.37 12 12 12 12-5.37 12-12S18.63 0 12 0zm6.024 9.726c.006.126.006.246.006.372 0 3.822-2.91 8.232-8.232 8.232a8.177 8.177 0 01-4.434-1.302 5.816 5.816 0 004.284-1.194 2.889 2.889 0 01-2.7-2.01 2.884 2.884 0 001.308-.048 2.891 2.891 0 01-2.322-2.838v-.036c.39.216.834.348 1.308.36a2.895 2.895 0 01-.9-3.858 8.208 8.208 0 005.964 3.024 2.894 2.894 0 014.926-2.64 5.859 5.859 0 001.836-.702 2.892 2.892 0 01-1.272 1.602 5.928 5.928 0 001.662-.456 5.657 5.657 0 01-1.434 1.494z"
					 ></path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6" >
				  <path
					 d="M11.995 2.162c3.205 0 3.581.014 4.848.071 1.171.053 1.805.248 2.228.415.562.219.962.476 1.381.895.42.419.681.819.896 1.38.162.425.361 1.058.414 2.23.057 1.266.071 1.642.071 4.847 0 3.205-.014 3.58-.071 4.848-.053 1.171-.248 1.804-.414 2.228a3.722 3.722 0 01-.896 1.381c-.419.42-.819.681-1.38.895-.424.162-1.058.362-2.23.415-1.266.057-1.642.071-4.847.071-3.205 0-3.58-.014-4.847-.071-1.172-.053-1.805-.248-2.229-.415a3.723 3.723 0 01-1.38-.895 3.664 3.664 0 01-.896-1.38c-.162-.425-.362-1.058-.414-2.23-.058-1.266-.072-1.642-.072-4.847 0-3.205.014-3.581.072-4.848.052-1.171.247-1.804.414-2.228.219-.562.476-.962.895-1.381a3.665 3.665 0 011.381-.895c.424-.162 1.057-.362 2.229-.415 1.266-.062 1.647-.071 4.847-.071zm0-2.162C8.738 0 8.33.014 7.048.071 5.77.13 4.9.333 4.138.63a5.847 5.847 0 00-2.124 1.385A5.95 5.95 0 00.63 4.138C.333 4.9.129 5.771.07 7.052.014 8.33 0 8.738 0 11.995c0 3.257.014 3.667.071 4.948.058 1.276.262 2.147.558 2.914.304.79.719 1.457 1.385 2.124a5.95 5.95 0 002.124 1.386c.762.295 1.633.5 2.914.557 1.281.057 1.686.071 4.948.071 3.262 0 3.667-.014 4.948-.071 1.276-.057 2.147-.262 2.914-.557a5.847 5.847 0 002.124-1.386 5.949 5.949 0 001.385-2.124c.296-.762.5-1.633.558-2.914.057-1.281.071-1.686.071-4.948 0-3.262-.014-3.666-.071-4.947-.058-1.277-.262-2.148-.558-2.915a5.846 5.846 0 00-1.385-2.123A5.95 5.95 0 0019.862.624c-.762-.295-1.633-.5-2.914-.557C15.662.014 15.252 0 11.995 0z"
					 ></path>
				  <path
					 d="M11.995 5.838a6.163 6.163 0 10.002 12.326 6.163 6.163 0 00-.002-12.326zm0 10.157a4 4 0 110-7.999 4 4 0 010 8zM18.4 7.029a1.438 1.438 0 100-2.877 1.438 1.438 0 000 2.877z"
					 ></path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
				  <path
					 d="M23.495 6.145a3.038 3.038 0 00-2.127-2.14C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.382.505c-1.036.272-1.84 1.09-2.127 2.14C0 8.027 0 11.955 0 11.955s0 3.927.505 5.809a3.037 3.037 0 002.127 2.14C4.5 20.41 12 20.41 12 20.41s7.5 0 9.382-.505c1.036-.272 1.84-1.09 2.127-2.14.491-1.882.491-5.81.491-5.81s0-3.927-.505-5.809zm-13.95 9.382V8.382l6.273 3.572-6.273 3.573z">
				  </path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6" >
				  <path
					 d="M5.99 0L1.7 4.286v15.428h5.147V24l4.29-4.286h3.431L22.29 12V0H5.99zm14.583 11.143l-3.431 3.428H13.71l-3.002 3v-3h-3.86V1.714h13.725v9.429z">
				  </path>
				  <path d="M18 4.714h-1.716v5.143H18V4.714zm-4.719 0h-1.715v5.143h1.715V4.714z"></path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
				  <path
					 d="M14.528 16.093c-.535.535-1.692.728-2.507.728-.814 0-1.971-.193-2.507-.728a.31.31 0 00-.45 0 .31.31 0 000 .45c.857.857 2.486.921 2.979.921.471 0 2.121-.064 2.978-.921a.31.31 0 000-.45c-.171-.107-.385-.107-.493 0zM10.5 13.264a1.244 1.244 0 10-2.487.002 1.244 1.244 0 002.487-.002z">
				  </path>
				  <path
					 d="M12 0C5.379 0 0 5.379 0 12s5.379 12 12 12 12-5.379 12-12c.021-6.621-5.357-12-12-12zm6.964 13.607c.022.171.043.343.043.536 0 2.7-3.129 4.886-7.007 4.886-3.879 0-7.007-2.208-7.007-4.908 0-.171.021-.364.043-.535A1.755 1.755 0 014.007 12c0-.964.793-1.757 1.757-1.757.472 0 .9.193 1.222.493 1.221-.879 2.893-1.436 4.757-1.479l.9-4.178a.312.312 0 01.128-.193.298.298 0 01.236-.043l2.914.621c.215-.407.622-.707 1.115-.707a1.244 1.244 0 010 2.486 1.25 1.25 0 01-1.243-1.2l-2.614-.557-.793 3.75c1.843.064 3.493.621 4.692 1.478.322-.3.75-.493 1.222-.493.964 0 1.757.793 1.757 1.758-.043.75-.471 1.35-1.093 1.628z">
				  </path>
				  <path d="M14.764 12a1.244 1.244 0 10.002 2.487A1.244 1.244 0 0014.764 12z"></path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg fill="currentColor" viewBox="0 0 24 24" width="1em" height="1em" class="h-6 w-6">
				  <path
					 d="M21.143 0H2.857A2.86 2.86 0 000 2.857v18.286A2.86 2.86 0 002.857 24h18.286A2.86 2.86 0 0024 21.143V2.857A2.86 2.86 0 0021.143 0zM18.86 10.47a4.28 4.28 0 01-3.977-1.916v6.593a4.872 4.872 0 11-4.873-4.873c.102 0 .202.01.302.016v2.4c-.1-.011-.199-.03-.302-.03a2.487 2.487 0 000 4.974c1.374 0 2.587-1.082 2.587-2.456l.024-11.196h2.297a4.279 4.279 0 003.944 3.82v2.668"
					 ></path>
			   </svg>
			</a>
		 </li>
		 <li>
			<a href="">
			   <svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
				  <path
					 d="M14.268 10.07c-.696 0-1.246.593-1.246 1.33 0 .74.562 1.332 1.246 1.332.695 0 1.245-.593 1.245-1.331s-.562-1.331-1.245-1.331zm-4.456 0c-.696 0-1.245.593-1.245 1.33 0 .74.561 1.332 1.245 1.332.696 0 1.245-.593 1.245-1.331.012-.738-.55-1.331-1.245-1.331z">
				  </path>
				  <path
					 d="M20.2 0H3.842C2.463 0 1.34 1.113 1.34 2.481V18.7c0 1.368 1.123 2.481 2.502 2.481h13.844l-.647-2.215 1.562 1.428 1.477 1.344L22.715 24V2.481A2.514 2.514 0 0020.2 0zm-4.712 15.673s-.44-.52-.805-.968c1.599-.448 2.21-1.428 2.21-1.428a7.01 7.01 0 01-1.405.714 8.806 8.806 0 01-1.77.52 8.622 8.622 0 01-3.162-.012 9.349 9.349 0 01-1.794-.52 7.17 7.17 0 01-.891-.412c-.037-.024-.073-.036-.11-.06-.024-.012-.037-.024-.049-.024-.22-.121-.342-.206-.342-.206s.586.956 2.137 1.416c-.367.46-.818.992-.818.992-2.698-.084-3.723-1.827-3.723-1.827 0-3.861 1.757-6.996 1.757-6.996 1.758-1.295 3.418-1.258 3.418-1.258l.123.145c-2.198.617-3.199 1.573-3.199 1.573s.269-.145.72-.339c1.307-.568 2.344-.714 2.771-.762.074-.012.135-.024.208-.024a10.417 10.417 0 012.466-.025c1.16.134 2.405.472 3.674 1.15 0 0-.964-.907-3.04-1.525l.172-.193s1.672-.037 3.418 1.258c0 0 1.758 3.135 1.758 6.996 0-.012-1.026 1.73-3.724 1.815z">
				  </path>
			   </svg>
			</a>
		 </li>
	  </ul>
   </div>
   <div class="col-lg-6 text-right">
	  <div class="row">
		 <div class="col-lg-6"></div>
		 <div class="col-lg-6">
			<div class="row">
			   <div class="col-lg-6 col-sm-7 col-5">
				  <p class="text-uppercase p-lg-small ligt-gray text-white pb-1">
					 LANGUAGE
				  </p>
				  <select class="form-select change-select px-1" aria-label="Default select example">
					 <option selected="">English</option>
					 <option value="1">Sinhala</option>
					 <option value="2">Tamil</option>
					 <option value="3">Arabic</option>
				  </select>
			   </div>
			   <div class="col-lg-6 col-sm-5 col-7">
				  <p class="text-uppercase p-lg-small ligt-gray text-white pb-1">
					 COUNTRY
				  </p>
				  <select class=" change-select px-1" aria-label="Default select example">
					 <option selected>North America</option>
					 <option value="1">United States</option>
					 <option value="2">Canada</option>
					 <option value="3">Europe</option>
				  </select>
			   </div>
			</div>
		 </div>
	  </div>
   </div>
</div>
<div class="row d-block d-sm-none">
   <div class="col-12 mobile-link  pb-4">


	  <div class="accordion" id="accordionExample">
		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingOne">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="false"
				  aria-controls="collapseOne">
				  CONTACT
			   </button>
			</h2>
			<div id="collapseOne" class="accordion-collapse collapse " aria-labelledby="headingOne"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">Company</a></li>
					 <li><a href="">Customer Support</a></li>
					 <li><a href="">Submit a Request</a></li>
					 <li><a href="">Support Center</a></li>
				  </ul>
			   </div>
			</div>
		 </div>
		 <div>



		 </div>
		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingTwo">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false"
				  aria-controls="collapseTwo">
				  ABOUT NZXT
			   </button>
			</h2>
			<div id="collapseTwo" class="accordion-collapse collapse " aria-labelledby="headingTwo"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">Founder Q &amp; A</a></li>
					 <li><a href="">Careers</a></li>
					 <li><a href="">Customer Reviews</a></li>
				  </ul>
			   </div>
			</div>
		 </div>

		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingThee">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false"
				  aria-controls="collapseThree">
				  COMMUNITY
			   </button>
			</h2>
			<div id="collapseThree" class="accordion-collapse collapse " aria-labelledby="headingThee"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">Our Discord
						</a></li>
					 <li><a href="">Newsroom & Blog</a></li>

				  </ul>
			   </div>
			</div>
		 </div>

		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingFour">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseFour" aria-expanded="false"
				  aria-controls="collapseFour">
				  SOFTWARE
			   </button>
			</h2>
			<div id="collapseFour" class="accordion-collapse collapse " aria-labelledby="headingFour"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">CAM</a></li>
					 <li><a href="">CAM Feedback</a></li>
				  </ul>
			   </div>
			</div>
		 </div>
		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingFive">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseFive" aria-expanded="false"
				  aria-controls="collapseFive">
				  ACCOUNT
			   </button>
			</h2>
			<div id="collapseFive" class="accordion-collapse collapse " aria-labelledby="headingFive"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">Manage Your Account</a></li>

				  </ul>
			   </div>
			</div>
		 </div>
		 <div class="accordion-item  border-0 p-0 ">
			<h2 class="bg-footer-black accordion-header bg-black  mb-2 border-bottom border-white "
			   id="headingSix">
			   <button class="accordion-button text-uppercase h6-size fw-bolder text-white pb-2  px-0 py-3"
				  type="button" data-bs-toggle="collapse" data-bs-target="#collapseSix" aria-expanded="false"
				  aria-controls="collapseSix">
				  NZXT STORE
			   </button>
			</h2>
			<div id="collapseSix" class="accordion-collapse collapse " aria-labelledby="headingSix"
			   data-bs-parent="#accordionExample">
			   <div class="accordion-body py-1 px-1">
				  <ul class="footer-ul">
					 <li><a href="">NZXT BLD PC</a></li>
					 <li><a href="">Refurbished Builds</a></li>
					 <li><a href="">FAQ</a></li>
					 <li><a href="">Find a Retailer</a></li>
				  </ul>
			   </div>
			</div>
		 </div>
	  </div>


   </div>
</div>





<div class="d-none d-sm-block">


   <div class="row px-lg-4 ">
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3 ">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">CONTACT</h6>
		 <ul class="footer-ul">
			<li><a href="">Company</a></li>
			<li><a href="">Customer Support</a></li>
			<li><a href="">Submit a Request</a></li>
			<li><a href="">Support Center</a></li>
		 </ul>
	  </div>
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">ABOUT NZXT</h6>
		 <ul class="footer-ul">
			<li><a href="">Founder Q & A</a></li>
			<li><a href="">Careers</a></li>
			<li><a href="">Customer Reviews</a></li>
		 </ul>
	  </div>
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">COMMUNITY</h6>
		 <ul class="footer-ul">
			<li><a href="">Our Discord</a></li>
			<li><a href="">Newsroom & Blog</a></li>
		 </ul>
	  </div>
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">SOFTWARE</h6>
		 <ul class="footer-ul">
			<li><a href="">CAM</a></li>
			<li><a href="">CAM Feedback</a></li>
		 </ul>
	  </div>
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">ACCOUNT</h6>
		 <ul class="footer-ul">
			<li><a href="">Manage Your Account</a></li>
		 </ul>
	  </div>
	  <div class="col-lg-2 col-sm-4 col-6 pt-lg-2 pt-sm-3 pt-3">
		 <h6 class="text-uppercase h6-size fw-bolder text-white pb-2">NZXT STORE</h6>
		 <ul class="footer-ul">
			<li><a href="">NZXT BLD PC</a></li>
			<li><a href="">Refurbished Builds</a></li>
			<li><a href="">FAQ</a></li>
			<li><a href="">Find a Retailer</a></li>
		 </ul>
	  </div>
   </div>
</div>



<div class="col-lg-6 social-media d-block text-center d-sm-none">
   <ul>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
			   <path
				  d="M24 12.073C24 5.405 18.628 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047v-2.66c0-3.025 1.79-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.971H15.83c-1.49 0-1.955.931-1.955 1.886v2.264h3.328l-.532 3.49h-2.796V24C19.613 23.095 24 18.1 24 12.073z">
			   </path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
			   <path
				  d="M12 0C5.37 0 0 5.37 0 12s5.37 12 12 12 12-5.37 12-12S18.63 0 12 0zm6.024 9.726c.006.126.006.246.006.372 0 3.822-2.91 8.232-8.232 8.232a8.177 8.177 0 01-4.434-1.302 5.816 5.816 0 004.284-1.194 2.889 2.889 0 01-2.7-2.01 2.884 2.884 0 001.308-.048 2.891 2.891 0 01-2.322-2.838v-.036c.39.216.834.348 1.308.36a2.895 2.895 0 01-.9-3.858 8.208 8.208 0 005.964 3.024 2.894 2.894 0 014.926-2.64 5.859 5.859 0 001.836-.702 2.892 2.892 0 01-1.272 1.602 5.928 5.928 0 001.662-.456 5.657 5.657 0 01-1.434 1.494z"
				  ></path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6" >
			   <path
				  d="M11.995 2.162c3.205 0 3.581.014 4.848.071 1.171.053 1.805.248 2.228.415.562.219.962.476 1.381.895.42.419.681.819.896 1.38.162.425.361 1.058.414 2.23.057 1.266.071 1.642.071 4.847 0 3.205-.014 3.58-.071 4.848-.053 1.171-.248 1.804-.414 2.228a3.722 3.722 0 01-.896 1.381c-.419.42-.819.681-1.38.895-.424.162-1.058.362-2.23.415-1.266.057-1.642.071-4.847.071-3.205 0-3.58-.014-4.847-.071-1.172-.053-1.805-.248-2.229-.415a3.723 3.723 0 01-1.38-.895 3.664 3.664 0 01-.896-1.38c-.162-.425-.362-1.058-.414-2.23-.058-1.266-.072-1.642-.072-4.847 0-3.205.014-3.581.072-4.848.052-1.171.247-1.804.414-2.228.219-.562.476-.962.895-1.381a3.665 3.665 0 011.381-.895c.424-.162 1.057-.362 2.229-.415 1.266-.062 1.647-.071 4.847-.071zm0-2.162C8.738 0 8.33.014 7.048.071 5.77.13 4.9.333 4.138.63a5.847 5.847 0 00-2.124 1.385A5.95 5.95 0 00.63 4.138C.333 4.9.129 5.771.07 7.052.014 8.33 0 8.738 0 11.995c0 3.257.014 3.667.071 4.948.058 1.276.262 2.147.558 2.914.304.79.719 1.457 1.385 2.124a5.95 5.95 0 002.124 1.386c.762.295 1.633.5 2.914.557 1.281.057 1.686.071 4.948.071 3.262 0 3.667-.014 4.948-.071 1.276-.057 2.147-.262 2.914-.557a5.847 5.847 0 002.124-1.386 5.949 5.949 0 001.385-2.124c.296-.762.5-1.633.558-2.914.057-1.281.071-1.686.071-4.948 0-3.262-.014-3.666-.071-4.947-.058-1.277-.262-2.148-.558-2.915a5.846 5.846 0 00-1.385-2.123A5.95 5.95 0 0019.862.624c-.762-.295-1.633-.5-2.914-.557C15.662.014 15.252 0 11.995 0z"
				  ></path>
			   <path
				  d="M11.995 5.838a6.163 6.163 0 10.002 12.326 6.163 6.163 0 00-.002-12.326zm0 10.157a4 4 0 110-7.999 4 4 0 010 8zM18.4 7.029a1.438 1.438 0 100-2.877 1.438 1.438 0 000 2.877z"
				  ></path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
			   <path
				  d="M23.495 6.145a3.038 3.038 0 00-2.127-2.14C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.382.505c-1.036.272-1.84 1.09-2.127 2.14C0 8.027 0 11.955 0 11.955s0 3.927.505 5.809a3.037 3.037 0 002.127 2.14C4.5 20.41 12 20.41 12 20.41s7.5 0 9.382-.505c1.036-.272 1.84-1.09 2.127-2.14.491-1.882.491-5.81.491-5.81s0-3.927-.505-5.809zm-13.95 9.382V8.382l6.273 3.572-6.273 3.573z">
			   </path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6" >
			   <path
				  d="M5.99 0L1.7 4.286v15.428h5.147V24l4.29-4.286h3.431L22.29 12V0H5.99zm14.583 11.143l-3.431 3.428H13.71l-3.002 3v-3h-3.86V1.714h13.725v9.429z">
			   </path>
			   <path d="M18 4.714h-1.716v5.143H18V4.714zm-4.719 0h-1.715v5.143h1.715V4.714z"></path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
			   <path
				  d="M14.528 16.093c-.535.535-1.692.728-2.507.728-.814 0-1.971-.193-2.507-.728a.31.31 0 00-.45 0 .31.31 0 000 .45c.857.857 2.486.921 2.979.921.471 0 2.121-.064 2.978-.921a.31.31 0 000-.45c-.171-.107-.385-.107-.493 0zM10.5 13.264a1.244 1.244 0 10-2.487.002 1.244 1.244 0 002.487-.002z">
			   </path>
			   <path
				  d="M12 0C5.379 0 0 5.379 0 12s5.379 12 12 12 12-5.379 12-12c.021-6.621-5.357-12-12-12zm6.964 13.607c.022.171.043.343.043.536 0 2.7-3.129 4.886-7.007 4.886-3.879 0-7.007-2.208-7.007-4.908 0-.171.021-.364.043-.535A1.755 1.755 0 014.007 12c0-.964.793-1.757 1.757-1.757.472 0 .9.193 1.222.493 1.221-.879 2.893-1.436 4.757-1.479l.9-4.178a.312.312 0 01.128-.193.298.298 0 01.236-.043l2.914.621c.215-.407.622-.707 1.115-.707a1.244 1.244 0 010 2.486 1.25 1.25 0 01-1.243-1.2l-2.614-.557-.793 3.75c1.843.064 3.493.621 4.692 1.478.322-.3.75-.493 1.222-.493.964 0 1.757.793 1.757 1.758-.043.75-.471 1.35-1.093 1.628z">
			   </path>
			   <path d="M14.764 12a1.244 1.244 0 10.002 2.487A1.244 1.244 0 0014.764 12z"></path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg fill="currentColor" viewBox="0 0 24 24" width="1em" height="1em" class="h-6 w-6">
			   <path
				  d="M21.143 0H2.857A2.86 2.86 0 000 2.857v18.286A2.86 2.86 0 002.857 24h18.286A2.86 2.86 0 0024 21.143V2.857A2.86 2.86 0 0021.143 0zM18.86 10.47a4.28 4.28 0 01-3.977-1.916v6.593a4.872 4.872 0 11-4.873-4.873c.102 0 .202.01.302.016v2.4c-.1-.011-.199-.03-.302-.03a2.487 2.487 0 000 4.974c1.374 0 2.587-1.082 2.587-2.456l.024-11.196h2.297a4.279 4.279 0 003.944 3.82v2.668"
				  ></path>
			</svg>
		 </a>
	  </li>
	  <li>
		 <a href="">
			<svg width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24" class="h-6 w-6">
			   <path
				  d="M14.268 10.07c-.696 0-1.246.593-1.246 1.33 0 .74.562 1.332 1.246 1.332.695 0 1.245-.593 1.245-1.331s-.562-1.331-1.245-1.331zm-4.456 0c-.696 0-1.245.593-1.245 1.33 0 .74.561 1.332 1.245 1.332.696 0 1.245-.593 1.245-1.331.012-.738-.55-1.331-1.245-1.331z">
			   </path>
			   <path
				  d="M20.2 0H3.842C2.463 0 1.34 1.113 1.34 2.481V18.7c0 1.368 1.123 2.481 2.502 2.481h13.844l-.647-2.215 1.562 1.428 1.477 1.344L22.715 24V2.481A2.514 2.514 0 0020.2 0zm-4.712 15.673s-.44-.52-.805-.968c1.599-.448 2.21-1.428 2.21-1.428a7.01 7.01 0 01-1.405.714 8.806 8.806 0 01-1.77.52 8.622 8.622 0 01-3.162-.012 9.349 9.349 0 01-1.794-.52 7.17 7.17 0 01-.891-.412c-.037-.024-.073-.036-.11-.06-.024-.012-.037-.024-.049-.024-.22-.121-.342-.206-.342-.206s.586.956 2.137 1.416c-.367.46-.818.992-.818.992-2.698-.084-3.723-1.827-3.723-1.827 0-3.861 1.757-6.996 1.757-6.996 1.758-1.295 3.418-1.258 3.418-1.258l.123.145c-2.198.617-3.199 1.573-3.199 1.573s.269-.145.72-.339c1.307-.568 2.344-.714 2.771-.762.074-.012.135-.024.208-.024a10.417 10.417 0 012.466-.025c1.16.134 2.405.472 3.674 1.15 0 0-.964-.907-3.04-1.525l.172-.193s1.672-.037 3.418 1.258c0 0 1.758 3.135 1.758 6.996 0-.012-1.026 1.73-3.724 1.815z">
			   </path>
			</svg>
		 </a>
	  </li>
   </ul>
</div>




<div class="row pt-lg-5 mt-lg-5 px-lg-4 copy-right">
   <div class="col-lg-3 col-sm-4">
	  <p class="p-lg-small text-white">© NZXT Inc. 2022 All Rights Reserved</p>
   </div>
   <div class="col-lg-4 col-sm-8">
	  <div class="row">
		 <div class="col-lg-2 col-sm-2">
			<p class="p-lg-small text-white"><a class="ligt-gray white" href="">Legal</a></p>
		 </div>
		 <div class="col-lg-3 col-sm-3">
			<p class="p-lg-small text-white"><a class="ligt-gray white" href="">Privacy Policy</a></p>
		 </div>
		 <div class="col-lg-7 col-sm-7">
			<p class="p-lg-small text-white"><a class="ligt-gray white" href=""> Manage Cookie Preferences</a></p>
		 </div>
	  </div>
   </div>
   <div class="col-lg-5"></div>
</div>
</div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/inc/js/bootstrap.min.js "></script>
<script src="<?php echo get_template_directory_uri(); ?>/inc/js/owl.carousel.min.js "></script>
<script src="<?php echo get_template_directory_uri(); ?>/inc/js/anime.js"></script>
<script defer src="https://use.fontawesome.com/releases/v5.15.4/js/all.js"
integrity="sha384-rOA1PnstxnOBLzCLMcre8ybwbTmemjzdNlILg8O7z1lUkLXozs4DHonlDtnE7fpc" crossorigin="anonymous"></script>

<?php wp_footer(); ?>

</body>
</html>
	