...
(function ( window, $, undefined ) {
	...
	$.yit_panel_typography.prototype = {
		...
		_initEvents: function () {
			...
			// Font size, font unit.
			$( [elements.size, elements.unit] ).each(
				function () {
					$( this ).on(
						'change',
						function () {
		...
		if ( typeof options === 'string' ) {
			...
			this.each(
				function () {
					...
					if ( ! instance ) {
						console.error( "cannot call methods on yit_checkout prior to initialization; attempted to call method '" + options + "'" );
						return;
					}
