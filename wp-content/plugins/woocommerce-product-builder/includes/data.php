...
//		echo '<pre>'.print_r($data_session,true).'</pre>';
	...
	public function enqueue_scripts() {
		...
		if ( VI_WOOPBTEMPLATE_STYLE === 'ajax' ) {
			...
			ob_start();
			wpb_get_template( 'ajax-parts/product.php', [ 'enable_multi_select' => $enable_multi_select, 'qty_field' => $qty_field ] );
		...
		if ( ! empty( $_REQUEST ) ) {
			foreach ( $_REQUEST as $key => $value ) {
				if ( 0 === strpos( $key, 'filter_' ) ) {
					...
					$query_type = ! empty( $_REQUEST[ 'query_type_' . $attribute ] ) && in_array( $_GET[ 'query_type_' . $attribute ], [ 'and', 'or' ], true )
	...
	public function get_detail_step( $step ) {
		if ( ! empty( $step ) && is_array( $step ) ) {
			foreach ( $step as $pid => $data ) {
				...
				$step[ $pid ]['title']    = $product->get_name();
