...
class VI_WPRODUCTBUILDER_FrontEnd_Ajax {
	...
	public function ajax() {
		if ( ! check_ajax_referer( 'woopb_nonce', 'nonce' ) ) {
			wp_die();
		}
		...
		if ( isset( $_POST['step'] ) ) {
			...
			if ( ! empty( $_POST['search'] ) ) {
				...
				foreach ( $products as $product ) {
					wpb_get_template( 'ajax-parts/product-modal.php', [ 'post_id' => $this->post_id, 'step' => $step ] );
				}
				$html = ob_get_clean();
	...
	public function get_session() {
		...
		wp_send_json_success( [ 'steps' => $steps, 'total' => $this->get_session_total(), 'checkDepend' => $this->check_depend() ] );
	}
	...
	public function add_item() {
		if ( isset( $_POST['step'] ) ) {
			...
			$atc_data = sanitize_text_field( $_POST['data'] );
			parse_str( $atc_data, $data );

			if ( ! empty( $data ) && is_array( $data ) ) {
				foreach ( $data as $key => $value ) {
					if ( ! in_array( $key, [ 'quantity', 'variation_id', 'product_id' ] ) && 'attribute_' !== substr( $key, 0, 10 ) ) {
						unset( $data[ $key ] );
					}
				}
			}
	...
	public function check_depend() {
		...
		$selected_data      = wc()->session->get( $this->session_key );
		$attr_compatible    = $params['attr_compatible'];
		$product_compatible = $params['product_compatible'];
	...
	public function change_quantity() {
		if ( isset( $_POST['product_id'], $_POST['quantity'] ) ) {
			...
			if ( isset( $session[ $this->step ][ $pid ]['quantity'] ) ) {
				...
				wp_send_json_success( [ 'step' => $step, 'total' => $total ] );
			}
		}
	}
	...
	public function download_pdf() {
		...
		try {
			$mpdf = new \Mpdf\Mpdf( [ 'tempDir' => WP_CONTENT_DIR . '/cache' ] );
