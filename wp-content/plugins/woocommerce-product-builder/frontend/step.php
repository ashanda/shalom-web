...
//			add_action( 'woocommerce_product_builder_before_steps_panel', [ $this, 'navigation' ], 12, 3 );
	...
	public function woocommerce_product_builder_single_variation( $post_id ) {
		...
		wpb_get_template( 'single/variation-add-to-cart-button.php', array( 'post_id' => $post_id, 'button_icon' => $button_icon ) );
	}
            ...
            <div class="woopb-sort-by-inner">
                ...
                <select class="woopb-sort-by-button woopb-button">
					...
					foreach ( $sort_by_events as $k => $sort_by_event ) { ?>
                        <option <?php selected( $current, $k ) ?> value="<?php echo add_query_arg( array( 'sort_by' => $k ) ) ?>">
...
//		$post_id = $post->ID;
            ...
            <div class="woopb-added-footer">
				...
				if ( $this->settings->has_step_added( $post_id ) || $this->get_data( $post_id, 'add_to_cart_always_show' ) ) {
                    ...
                    <form method="POST" action="<?php echo wc_get_cart_url() ?>" class="woopb-form-cart-now">
						<?php wp_nonce_field( 'woopb_nonce', '_nonce' ) ?>
	...
	public function simple_add_to_cart( $post_id ) {
		$button_icon = $this->settings->get_button_icon();
		wpb_get_template( 'single/add-to-cart-simple.php', array( 'post_id' => $post_id, 'button_icon' => $button_icon ) );
	}
...
//		}
	...
	public function init_scripts() {
		...
		wp_register_script( 'woocommerce-product-builder', VI_WPRODUCTBUILDER_JS . 'woocommerce-product-builder' . $suffix, array( 'jquery', 'photoswipe', 'wc-single-product' ), VI_WPRODUCTBUILDER_VERSION );
	...
	public function product_builder_content_single_page( $id ) {
		...
		if ( $load_single ) {
			...
			wpb_get_template( $single_template, array( 'id' => $id, 'products' => $products, 'max_page' => $max_page, 'step_error' => $step_error ) );
		}
	}
...
//	AJAX Template style
	...
	public function ajax_product_builder_search_form( $post_id ) {
		if ( $this->get_data( $post_id, 'search_product_form' ) ) {
			?>
            <input type="text" class="woopb-search" placeholder="<?php esc_html_e( 'Search...', 'woocommerce-product-builder' ); ?>">
			<?php
		}
	}
