...
class YWRAQ_Request_List_Table extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'status':
                $post_edit = add_query_arg(  array('post' => $item->get_id(), 'action' => 'edit'), admin_url() . 'post.php' );
	...
	/**
	 * Return the subscription total number
	 *
	 * @param string $status Status.
	 *
	 * @return array|object|null
	 */
	protected function get_subscription_total_number( $status = 'all' ) {
		global $wpdb;
	...
	public function prepare_items() {
		...
		$per_page = 20;
		$status   = apply_filters( 'ywraq_request_list_status_filter', ywraq_get_quote_status_list() );
		...
		if ( $get_from_query ) {
			...
			$quotes      = $this->get_subscription_total_number( $status );
