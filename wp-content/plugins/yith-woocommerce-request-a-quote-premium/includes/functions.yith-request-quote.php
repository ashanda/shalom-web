...
if ( ! function_exists( 'yith_ywraq_render_button' ) ) {
	...
	function yith_ywraq_render_button( $product_id = false, $args = array() ) {
		...
		$style_button = isset( $args['style'] ) ? $args['style'] : $style_button;
		...
		if ( ( function_exists( 'yith_wapo_product_has_blocks' ) || class_exists( 'YITH_WAPO_Type' ) ) && ( ! is_product() || apply_filters(
			...
			if ( class_exists( 'YITH_WAPO_Type' ) ) {
				...
				$has_addons = yith_wapo_product_has_blocks( $product_id );
			}
			if ( ! empty( $has_addons ) ) {
		...
		if ( $product->is_type( 'yith-composite' ) && ( ! is_product() || apply_filters(
					'yith_ywraq_render_button_check_loop',
					in_array(
						$woocommerce_loop['name'],
						array(
		...
		wc_get_template(
			$template_button,
			apply_filters( 'ywraq_add_to_quote_args', $args ),
			'',
...
if ( ! function_exists( 'ywraq_get_label' ) ) {
	...
	function ywraq_get_label( $key, $echo = false ) {
		...
		switch ( $key ) {
			case 'product_added':
				$label = $option ? $option : apply_filters(
					'yith_ywraq_product_added_to_list_message',
					esc_html__( 'Product added to the list!', 'yith-woocommerce-request-a-quote' )
				);
				break;
			case 'browse_list':
				$label = $option ? $option : apply_filters(
					'ywraq_product_added_view_browse_list',
					esc_html__( 'Browse the list', 'yith-woocommerce-request-a-quote' )
				);
				break;
			case 'btn_link_text':
				$label = $option ? $option : apply_filters(
					'ywraq_product_add_to_quote',
					esc_html__( 'Add to Quote', 'yith-woocommerce-request-a-quote' )
				);
				break;
			case 'already_in_quote':
				$label = $option ? $option : apply_filters(
					'yith_ywraq_product_already_in_list_message',
					esc_html__( 'Product already in the list.', 'yith-woocommerce-request-a-quote' )
				);
				break;
			case 'accept':
				$label = get_option(
					'ywraq_accept_link_label',
					esc_html__( 'Accept', 'yith-woocommerce-request-a-quote' )
				);
				break;
			case 'reject':
				$label = get_option(
					'ywraq_reject_link_label',
					esc_html__( 'Reject', 'yith-woocommerce-request-a-quote' )
				);
				break;
			default:
				$label = '';
...
if ( ! function_exists( 'ywraq_get_message_after_request_quote_sending' ) ) {
	...
	function ywraq_get_message_after_request_quote_sending( $new_order = '' ) {
		...
		$quote_number = get_option(
			                'ywraq_enable_order_creation',
			                'yes'
		                ) === 'yes' ? apply_filters( 'ywraq_quote_number', $new_order ) : '';
		...
		$quote_number_link = empty( $quote_number ) ? '' : sprintf(
			'<a href="%1$s">#%2$s</a>',
			$quote_url,
			$quote_number
		);
		$quote_number_link = apply_filters(
...
if ( ! function_exists( 'ywraq_get_list_empty_message' ) ) {
	...
	function ywraq_get_list_empty_message() {
		...
		$empty_list_message_text = apply_filters(
		...
		if ( $raq_id ) {
			...
			$label_return_to_shop = apply_filters(
				'yith_ywraq_return_to_shop_after_sent_the_request_label',
				get_option( 'ywraq_return_to_shop_after_sent_the_request' )
			);
		} else {
			$shop_url             = ywraq_get_return_to_shop_url();
			$label_return_to_shop = apply_filters(
				'yith_ywraq_return_to_shop_label',
				get_option( 'ywraq_return_to_shop_label' )
			);
		}
...
if ( ! function_exists( 'yith_ywraq_show_button_in_single_page' ) ) {
	...
	function yith_ywraq_show_button_in_single_page() {
		...
		if ( ! $product->is_type( 'variable' ) && 'only' === get_option(
				'ywraq_button_out_of_stock',
				'hide'
			) && $product->is_in_stock() ) {
			return false;
		}
	...
	function ywraq_get_actions_menu( $quote ) {
		...
		if ( 'yes' === get_option( 'ywraq_enable_order_again', 'no' ) ) {
			...
			if ( ! $quote->has_status(
				...
				$quote_again_url = wp_nonce_url(
					add_query_arg(
						'raq_again',
						$quote->get_id(),
						YITH_Request_Quote_Premium()->get_raq_url( '' )
					),
					'ywraq-order_again'
				);
